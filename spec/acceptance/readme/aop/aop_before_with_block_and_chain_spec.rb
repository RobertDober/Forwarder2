...
describe Forwarder do
  describe "AOP" do 
    context "before_with_block_and_chain" do 
      let :lazy do
      ...
      context :forward do
        before do
          class << lazy
            extend Forwarder
            forward :drop_until, to_chain: [:data, :first], as: :drop_while, before_with_block: ->(&b){b.negated}
          end
        end
      ...
      context :forward_all do
        before do
          class << lazy
            extend Forwarder
            forward_all :drop_until, :xxx, to_chain: [:data, :first], as: :drop_while, before_with_block: ->(&b){b.negated}
          end
        end
